...
namespace Matrix
{
    ...
    public class QuadraticMatrix<T> : Matrix<T>, ICloneable
    {
        ...
        /// <summary>
        /// Add two quadratic matrix.
        /// </summary>
        /// <param name="leftMatrix">left argument</param>
        /// <param name="rigghtMatrix">right argument</param>
        /// <returns>Quadratic matrix.</returns>
        public static QuadraticMatrix<T> operator +(QuadraticMatrix<T> leftMatrix, QuadraticMatrix<T> rigghtMatrix)
        {
            CheckMatrix(leftMatrix);
            CheckMatrix(rigghtMatrix);
            if (leftMatrix.Height != rigghtMatrix.Height || leftMatrix.Width != rigghtMatrix.Width)
            {
                throw new ArgumentException("Matrix shuld be of same dimension.");
            }
            ...
            for (int i = 0; i < result.Height; i++)
            {
                for (int j = 0; j < result.Width; j++)
                {
                    try
                    {
                        dynamic first = result[i, j];
                        dynamic second = rigghtMatrix[i, j];
                        result[i, j] = first + second;
                    }
                    catch (Exception)
                    {
                        throw new InvalidOperationException($"Can't summ two values of {nameof(T)}");
                    }
                }
            }
        ...
        /// <summary>
        /// Add quadratic matrix to diagonal matrix.
        /// </summary>
        /// <param name="leftMatrix">quadratic matrix</param>
        /// <param name="rigghtMatrix">diagonal matrix</param>
        /// <returns>Quadratic matrix</returns>
        public static QuadraticMatrix<T> operator +(QuadraticMatrix<T> leftMatrix, DiagonalMatrix<T> rigghtMatrix)
        {
            CheckMatrix(leftMatrix);
            CheckMatrix(rigghtMatrix);
            if (leftMatrix.Height != rigghtMatrix.Height || leftMatrix.Width != rigghtMatrix.Width)
            ...
            for (int i = 0; i < result.Height; i++)
            {
                dynamic first = result[i, i];
                dynamic second = rigghtMatrix[i, i];
                result[i, i] = first + second;
            }
        ...
        /// <summary>
        /// Add diagonal matrix to quadratic matrix.
        /// </summary>
        /// <param name="leftMatrix">diagonal matrix</param>
        /// <param name="rigghtMatrix">quadratic matrix</param>
        /// <returns>Quadratic matrix.</returns>
        public static QuadraticMatrix<T> operator +(DiagonalMatrix<T> leftMatrix, QuadraticMatrix<T> rigghtMatrix)
        {
            CheckMatrix(leftMatrix);
            CheckMatrix(rigghtMatrix);
            return rigghtMatrix + leftMatrix;
        }
